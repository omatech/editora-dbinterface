...
class ReverseEngineerator extends DBInterfaceBase
{
    ...
    public function reverseEngineerEditora()
    {
        ...
        $return_array['attributes_json']=$this->getAttributes('J');
        ...
        $return_array['attributes_multi_lang_text']=$this->getAttributes('T', true);
        $return_array['attributes_multi_lang_file']=$this->getAttributes('F', true);
        $return_array['attributes_multi_lang_url']=$this->getAttributes('U', true);
        $return_array['attributes_multi_lang_image']=$this->getAttributes('I', true);
        $return_array['attributes_multi_lang_json']=$this->getAttributes('J', true);
    ...
    public function arrayToCode($rows)
    {
        ...
        foreach ($rows as $key=>$val) {
            ...
            } elseif ($key=='attributes_json') {
                $return_string .= $this->attributeArrayToCode('attributes_json', $val);
            ...
            } elseif ($key=='attributes_multi_lang_text') {
                $return_string .= $this->attributeArrayToCode('attributes_multi_lang_text', $val);
            ...
            } elseif ($key=='attributes_multi_lang_url') {
                $return_string .= $this->attributeArrayToCode('attributes_multi_lang_url', $val);
            } elseif ($key=='attributes_multi_json') {
                $return_string .= $this->attributeArrayToCode('attributes_multi_lang_json', $val);
            } elseif ($key=='lookups') {
    ...
    public function getRelationsArray()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                if (isset($row['multiple_child_class_id']) && !empty($row['multiple_child_class_id'])) {// we have at least one image size
    ...
    public function getAttributesClassesArray()
    {
        $return_array=array();
        $sql="select * from (
		select 0 es_rel, ca.class_id, ca.tab_id, ca.atri_id, ca.rel_id, ca.fila, 1 columna 
        ...
        foreach ($rows as $row) {
            ...
            if (isset($row['atri_id'])) {
                $atri_id=$row['atri_id']-$resta;
            }
            if (isset($row['rel_id'])) {
                $atri_id=$row['rel_id'];
            }
    ...
    public function getImagesSizesArray()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                if (isset($row['img_width']) || isset($row['img_height'])) {// we have at least one image size
                    $size_string='x';
