...
if ($options_array['to'] == 'db4' || $options_array['to'] == 'db5') {// let's backup the database first
    ...
    if (!isset($options_array['avoiddbbackup'])) {
        $cmd = "mysqldump --user=" . $options_array['dbuser'] . " --password=" . $options_array['dbpass'] . " --host=" . $options_array['dbhost'] . " " . $options_array['dbname'] . " omp_values omp_niceurl omp_static_text > " . sys_get_temp_dir() . "/backup_ompvalues_preimport_" . $options_array['dbname'] . "_$backup_date.sql";
